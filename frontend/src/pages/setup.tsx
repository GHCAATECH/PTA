...
export default function Setup() {
  ...
  const mutation = useMutation({
    mutationFn: async (e: NonNullable<Editor>) => {
      if (e.kind === "year") return api.addYear(e.value.trim());
      if (e.kind === "class")
        return e.id
          ? api.renameClass(e.id, e.value.trim())
          : api.addClass(e.value.trim());
      const active = setup.data?.years.find((y) => y.is_active);
      if (!active) throw new Error("Activate an academic year first");
  ...
  const activate = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success("Active academic year changed");
    },
    onError: (e) => toast.error(e.message),
  });
    ...
    onError: (error) =>
      toast.error(
        error.message.includes("foreign key")
          ? "Move or deactivate the students in this class before deleting it."
          : error.message,
      ),
  });
  if (setup.isLoading)
          ...
          {years.map((y) => (
            ...
            >
              ...
              <div className="flex-1">
                ...
                <p className="text-[11px] text-slate-500">Academic session</p>
              </div>
              {y.is_active ? (
      ...
      <Card>
        <CardHeader>
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">
              One fee for each academic year.
            </p>
          </div>
        </CardHeader>
        <CardContent>
          <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-indigo-600 to-blue-500 p-5 text-white">
            ...
            <p className="text-xs text-indigo-100">
              {active?.year ?? "No active year"} PTA fee
            </p>
      ...
      <Card className="xl:col-span-2">
        <CardHeader>
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">
              {classes.length} classes available for student placement.
            </p>
          </div>
          <Button
            size="sm"
      ...
      >
        <label>
          ...
          <input
            ...
            pattern={
              editor.kind === "year" ? "20[0-9]{2}/20[0-9]{2}" : undefined
            }
            value={value}
