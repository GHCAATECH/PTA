...
export default function Students() {
  ...
  const [students, setStudents] = useState<Student[]>([]),
    ...
    [statusFilter, setStatusFilter] = useState<Student["status"] | "ALL">("active"),
  ...
  const filtered = useMemo(
    ...
    [students, search, paymentFilter, classFilter, statusFilter],
  );
  const progressableStudents = filtered.filter((s) => s.status === "active");
    ...
    <div className="space-y-5">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex flex-wrap gap-2">
          {profile?.role === "administrator" && (
            <Button
              variant="outline"
              disabled={!progressableStudents.length || progressStudents.isPending}
              onClick={() => {
                if (
                  window.confirm(
                    `Progress ${progressableStudents.length} active student(s)? SHS 1 moves to SHS 2, SHS 2 moves to SHS 3, and SHS 3 students move to the archive list.`,
                  )
                ) {
                  progressStudents.mutate(progressableStudents.map((student) => student.id));
                }
              }}
            >
              {progressStudents.isPending ? (
      ...
      <Card className="overflow-hidden">
        ...
        <div className="flex flex-col gap-3 border-t border-slate-200/70 p-4 text-xs text-slate-500 sm:flex-row sm:items-center sm:justify-between dark:border-white/8">
          ...
          <span>{progressableStudents.length} active student(s) ready for progression</span>
        </div>
      </Card>
      <StudentForm
        open={open}
  ...
  return (
    <tr className="transition hover:bg-slate-50/70 dark:hover:bg-white/[.025]">
      ...
      <td className="px-4 py-3">
        <Badge className={studentStatusStyle[s.status]}>
      ...
      <td className="px-4 py-3">
        <Badge className={paymentStatusStyle[s.payment_status]}>
          {s.payment_status}
        </Badge>
      </td>
      <td className="px-4 py-3">
